...
public class BattleManager
{
	...
	public static int DOUBLE_ATTACK_CONSTANT = 4;
	...
	public static int[] normalBattleForecast(Unit atk, Unit dfd, int bodyPart,
			BattlegroundTile atkTile, BattlegroundTile dfdTile, bool canCounter)
	{
		int[] ret = new int[10];
		...
		ret[2] = Mathf.Max(0, ret[2]);
		ret[3] = Mathf.Max(0, atk.criticalHitRate() - dfd.criticalHitAvoid());
		...
		if (canCounter)
		{
			ret[6] = Mathf.Max(0, dfd.attackStrength() - atk.defense(atk.isUsingMagic(), 1));
			ret[7] = dfd.accuracy() - atk.avoidance(1);
			if (!(atk.canFly()))
			{
				ret[7] -= atkTile.avoidanceBonus();
			}
			ret[7] = Mathf.Max(0, ret[7]);
			ret[8] = Mathf.Max(0, dfd.criticalHitRate() - atk.criticalHitAvoid());
	...
	public static StandardBattleReport performBattleSequence(Unit atk, Unit dfd, int bodyPart,
			BattlegroundTile atkTile, BattlegroundTile dfdTile, BattleGround battle, bool canCounter)
	{
			...
			Ship dfdShip, int shipPart, WorldMapTile worldLocation, ShipBattleReport report)
	{
		...
		killer.incrementKills(drowned.Count);
		Dictionary<Nation, Nation> nationsInvolved = new Dictionary<Nation, Nation>();
		for (int q = 0; q < drowned.Count; q++)
		{
			Unit u = drowned[q];
			...
			u.deathSequence();
			if (nationsInvolved[u.getAffiliation()] == null)
			{
				nationsInvolved.Add(u.getAffiliation(), u.getAffiliation());
			}
		}
		...
		if (dfdShip.hasPlayerNationUnit() || dfdShip.isImportant())
		{
			...
			{
				Dictionary<Nation, Nation>.KeyCollection nations = nationsInvolved.Keys;
